...
export default function Profil() {
  ...
  const handleConfirmLogOut = async () => {
    try {
      ...
      navigate("/pagepublic", { replace: true }); // Redirige vers la page publique
    }
  };
  ...
  return (
    <>
      ...
      <div className="flex items-center gap-4 cursor-pointer hover:scale-105 duration-300" onClick={() => setIsOpenProfil(true)}>
        <img src={userPhoto || null} alt="Profil" className="w-8 h-8 sm:w-9 sm:h-9 rounded-full object-cover" />
      </div>
      ...
      <AnimatePresence>
        {confirmLogOut && (
          <motion.div className="fixed inset-0 z-50 flex items-center justify-center p-4" variants={backdropVariants} initial="hidden" animate="visible" exit="hidden" onClick={() => setConfirmLogOut(false)}>
            <div className="absolute top-0 inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center">
              <motion.div className="relative bg-white/95 backdrop-blur-lg rounded-2xl sm:rounded-3xl shadow-2xl border border-white/20 max-w-xs sm:max-w-md w-full mx-4 overflow-hidden" variants={modalVariants} initial="hidden" animate="visible" exit="exit" onClick={(e) => e.stopPropagation()}>
                <div className="bg-gradient-to-r from-blue-600 to-purple-500 p-4 sm:p-6 text-white relative">
                  <motion.button onClick={() => setConfirmLogOut(false)} className="absolute top-3 right-3 sm:top-4 sm:right-4 p-1.5 sm:p-2 hover:bg-white/20 rounded-full transition-colors" whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
                ...
                <div className="p-6 sm:p-8 text-center">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center">
                    <motion.button onClick={handleConfirmLogOut} className="px-6 sm:px-8 py-2.5 sm:py-3 bg-gradient-to-r from-blue-500 to-purple-400 text-white rounded-full font-semibold shadow-lg hover:shadow-xl transition-all duration-300 min-w-[100px] cursor-pointer text-sm sm:text-base order-2 sm:order-1" variants={buttonVariants} whileHover="hover" whileTap="tap">
                      Oui
                    </motion.button>
                    <motion.button onClick={() => setConfirmLogOut(false)} className="cursor-pointer px-6 sm:px-8 py-2.5 sm:py-3 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-full font-semibold shadow-lg hover:shadow-xl transition-all duration-300 min-w-[100px] text-sm sm:text-base order-1 sm:order-2" variants={buttonVariants} whileHover="hover" whileTap="tap">
                      Non
                    </motion.button>
                  </div>
                </div>
