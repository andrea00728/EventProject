...
const PaymentPage = ({ forfait, onClose }) => {
  ...
  // Charger les forfaits depuis le backend
  useEffect(() => {
    const fetchForfaits = async () => {
      try {
        ...
        // Map pour les icônes et styles en fonction du nom
        const iconMap = {
          freemium: { icon: Star, buttonColor: "bg-gray-500 hover:bg-gray-600", bgColor: "bg-gray-100", iconBg: "bg-gray-200", textColor: "text-gray-800" },
          starter: { icon: Clock, buttonColor: "bg-blue-500 hover:bg-blue-600", bgColor: "bg-blue-100", iconBg: "bg-blue-200", textColor: "text-blue-800" },
          pro: { icon: Users, buttonColor: "bg-green-500 hover:bg-green-600", bgColor: "bg-green-100", iconBg: "bg-green-200", textColor: "text-green-800" },
          premium: { icon: Gift, buttonColor: "bg-purple-500 hover:bg-purple-600", bgColor: "bg-purple-100", iconBg: "bg-purple-200", textColor: "text-purple-800" },
          gold: { icon: Crown, buttonColor: "bg-yellow-500 hover:bg-yellow-600", bgColor: "bg-yellow-100", iconBg: "bg-yellow-200", textColor: "text-yellow-800" },
        };

        // Transformer les données pour correspondre au format attendu
        const transformedForfaits = forfaitsData.reduce((acc, forfait) => {
          const nameLower = (forfait.nom || "").toLowerCase();
          const config = iconMap[nameLower] || {
            icon: Star,
          ...
          acc[nameLower] = {
            ...
            name: forfait.nom || "Sans nom",
            icon: config.icon,
            price: formatPrice(forfait.price),
            duration: getDurationText(forfait.validationduration),
            maxInvites: forfait.maxinvites ?? "Illimité",
            events: forfait.maxevents ?? "Illimité",
            features: forfait.fonctionnalite ? forfait.fonctionnalite.split(",") : ["Fonctionnalité non spécifiée"],
  ...
  const selectedForfait = forfaits[selectedPlan];
