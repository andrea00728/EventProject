...
import * as XLSX from 'xlsx'; // Assurez-vous d'installer le paquet 'xlsx'

// Composant de modal pour l'ajout/modification de forfaits (inchangé)
...
// Composant de modal de confirmation (inchangé)
...
// Composant principal de la page de gestion (mis à jour)
...
  const { darkMode } = useDarkMode();
  ...
  // Crée une référence pour l'élément input de type fichier
...
  };
  ...
  // Lit et traite le fichier sélectionné
      ...
      reader.onload = async (e) => {
        try {
          ...
          if (!Array.isArray(json) || json.length === 0) {
            setError("Le fichier importé est vide ou a un format incorrect.");
            setIsLoading(false);
            return;
          }
          ...
          for (const pkg of json) {
            if (!pkg.nom || !pkg.price || !pkg.maxevents || !pkg.maxinvites || !pkg.validationduration || !pkg.fonctionnalite || !pkg.ideal) {
              console.warn("Ligne de forfait ignorée car incomplète:", pkg);
              continue;
            }
            await addForfait(pkg);
          }

          await fetchPackages();
          console.log("Forfaits importés avec succès.");
        } catch (err) {
          console.error("Erreur lors du traitement du fichier:", err);
          setError("Erreur lors du traitement du fichier. Vérifiez le format (CSV/XLSX) et les en-têtes de colonnes.");
        } finally {
          setIsLoading(false);
        }
      };
...
              className="w-full sm:w-auto bg-gradient-to-r from-pink-500 to-fuchsia-600 hover:from-pink-600 hover:to-fuchsia-700 text-white font-bold py-2 px-6 rounded-full shadow-lg transition duration-300 ease-in-out transform hover:scale-105"
            >
              Importer
            </button>
            <input
