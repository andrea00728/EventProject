...
const DashboardGlobal = () => {
    ...
    const [goals, setGoals] = useState(() => {
        ...
        return savedGoals ? JSON.parse(savedGoals) : { monthlyEvents: 3, attendeesTarget: 50 };
    });
    ...
    const handleGoalChange = (key, increment) => {
      setGoals(prevGoals => {
        ...
        const updatedGoals = { ...prevGoals, [key]: newValue > 0 ? newValue : prevGoals[key] };
    ...
    const getPlanIcon = (planName) => {
        switch (planName.toLowerCase()) {
            case 'starter': return <Rocket className="w-12 h-12 text-blue-500" strokeWidth={2.5} />;
            case 'pro': return <Star className="w-12 h-12 text-purple-500" strokeWidth={2.5} />;
            case 'premium': return <Gem className="w-12 h-12 text-pink-500" strokeWidth={2.5} />;
            case 'gold': return <Crown className="w-12 h-12 text-yellow-600" strokeWidth={2.5} />;
            default: return null;
        }
    };
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <header className="mb-8 text-gray-900">
                    <div className="flex flex-col md:flex-row md:items-center md:gap-4">
                        <div className="flex items-center gap-4">
                            ...
                            <h1 className="text-4xl font-extrabold">Bienvenue sur votre Tableau de Bord,</h1>
                        </div>
                ...
                <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                    ...
                    <div className="bg-white rounded-xl shadow-lg p-6 border-l-4 border-indigo-600 hover:shadow-2xl transition-shadow duration-300">
                        <h2 className="text-sm font-medium text-gray-500 uppercase tracking-wide">Forfait Actuel</h2>
                        <div className="flex items-center justify-between mt-2">
                            <span className="text-3xl font-bold text-gray-900">{dashboardData.plan}</span>
                            {getPlanIcon(dashboardData.plan)}
                        </div>
                        <p className="text-xs text-gray-400 mt-1">Votre plan détermine vos fonctionnalités.</p>
                    </div>
                    ...
                    <div className="bg-white rounded-xl shadow-lg p-6 border-l-4 border-yellow-500 hover:shadow-2xl transition-shadow duration-300">
                        <h2 className="text-sm font-medium text-gray-500 uppercase tracking-wide">Points d'Événement</h2>
                        <div className="flex items-center justify-between mt-2">
                        ...
                        <p className="text-xs text-gray-400 mt-1">Utilisez-les pour créer de nouveaux événements.</p>
                    </div>
                    ...
                    <div className="bg-white rounded-xl shadow-lg p-6 border-l-4 border-green-500 hover:shadow-2xl transition-shadow duration-300">
                        <h2 className="text-sm font-medium text-gray-500 uppercase tracking-wide">Événements Créés</h2>
                        <div className="flex items-center justify-between mt-2">
                        ...
                        <p className="text-xs text-gray-400 mt-1">Nombre total d'événements que vous avez lancés.</p>
                    </div>
                    ...
                    <div className="bg-white rounded-xl shadow-lg p-6 border-l-4 border-pink-500 hover:shadow-2xl transition-shadow duration-300">
                        <h2 className="text-sm font-medium text-gray-500 uppercase tracking-wide">Événements à Venir</h2>
                        <div className="flex items-center justify-between mt-2">
                        ...
                        <p className="text-xs text-gray-400 mt-1">Événements planifiés dans le futur.</p>
                    </div>
                </section>

                {/* Quick Actions */}
                <section className="mb-8">
                    <h2 className="text-2xl font-semibold text-gray-800 mb-4">Actions Rapides</h2>
                ...
                <section className="bg-white rounded-xl shadow-lg p-6 border-t-4 border-indigo-500">
                    ...
                    <p className="text-gray-600 mb-4">Définissez et suivez vos objectifs pour rester motivé et organisé.</p>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 flex flex-col justify-between">
                            ...
                            <div className="flex justify-between items-center mt-2">
                                <p className="text-xs text-gray-500">Actuellement : {dashboardData.eventsCreated}</p>
                                <div className="flex space-x-2">
                                    <button onClick={() => handleGoalChange('monthlyEvents', -1)} className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-200 text-gray-700 hover:bg-gray-300 transition-colors duration-200" disabled={goals.monthlyEvents <= 1}>-</button>
                                    <button onClick={() => handleGoalChange('monthlyEvents', 1)} className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-200 text-gray-700 hover:bg-gray-300 transition-colors duration-200">+</button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 flex flex-col justify-between">
                            ...
                            <div className="flex justify-between items-center mt-2">
                                <p className="text-xs text-gray-500">Actuellement : 15 (exemple)</p>
                                <div className="flex space-x-2">
                                    <button onClick={() => handleGoalChange('attendeesTarget', -10)} className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-200 text-gray-700 hover:bg-gray-300 transition-colors duration-200" disabled={goals.attendeesTarget <= 10}>-</button>
                                    <button onClick={() => handleGoalChange('attendeesTarget', 10)} className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-200 text-gray-700 hover:bg-gray-300 transition-colors duration-200">+</button>
                                </div>
                            </div>
                        </div>
                    </div>
