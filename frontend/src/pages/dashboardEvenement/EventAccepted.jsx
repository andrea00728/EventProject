...
const EventAccept = () => {
  ...
  const handleCancelEvent = async (eventId) => {
    if (window.confirm("Êtes-vous sûr de vouloir annuler cet événement ? Il sera marqué comme annulé mais pas supprimé.")) {
      try {
                ...
                {hiddenEvents.map((event) => {
                  ...
                  return (
                    <div key={event.id} className="bg-white rounded-lg shadow-md p-6 border border-gray-200">
                      ...
                      <p className="text-gray-600 mb-1"><span className="font-medium">Date:</span> {new Date(event.date).toLocaleDateString("fr-FR")}</p>
                      <p className={`mb-3 font-medium ${textColor}`}><span className="font-medium">Statut:</span> {text}</p>
                      <button
            ...
            {events.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 items-stretch">
                {events.map((event, index) => {
                  ...
                  const { text, color, textColor } = getStatusTextAndColor(event.status);
                  return (
                    <div key={event.id} className="group bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 border border-slate-200 overflow-hidden transform hover:-translate-y-1 h-full flex flex-col">
