...
const CartItem = ({ item, updateQuantity, removeFromCart, formatPrice }) => (
  <div className="flex justify-between items-center border-b border-gray-200 pb-4">
    <div className="flex-1 flex gap-4 items-center">
      {item.photo ? (
        <img
          src={`${import.meta.env.VITE_API_BASE_URL}${item.photo.startsWith('/') ? '' : '/'}${item.photo}`}
          alt={item.name}
      ...
      <div>
        ...
        <div className="flex items-center gap-4 mt-2">
          ...
          <p
            ...
            aria-label={`Prix total ${formatPrice(item.price * item.quantity)} euros`}
          >
    ...
    <button
      onClick={() => {
        if (window.confirm(`Voulez-vous vraiment retirer "${item.name}" du panier ?`)) {
          ...
          <div className="flex flex-col h-full">
            ...
            {cart.length > 0 && (
              <div className="p-6 border-t border-gray-200 bg-white">
                <div
                  className="flex justify-between items-center text-xl font-semibold text-gray-900 mb-4"
                  aria-live="polite"
                >
                  <span>Total :</span>
                  <span
                    className="text-green-600"
                    aria-label={`Total du panier ${formatPrice(totalPrice)} euros`}
                  >
...
CartDrawer.propTypes = {
  ...
  onValidateOrder: PropTypes.func,
};
