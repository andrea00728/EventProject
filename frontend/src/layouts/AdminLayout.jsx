...
export default function AdminLayout() {
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <p className={`text-sm line-clamp-2 ${!item.read ? 'font-medium' : ''}`}>
                          {item.label}  {/* On va utiliser directement label préparé dans itemsForDropdown */}
                        </p>
  ...
  const AdminHeader = ({ currentPageName, darkMode }) => {
    ...
    const itemsForDropdown = messages.map(msg => ({
      ...msg,
      label: `${msg.firstName} ${msg.lastName} : ${msg.message}`,  // Affiche prénom nom + message complet
    }));
