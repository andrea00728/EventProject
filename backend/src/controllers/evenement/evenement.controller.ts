...
export class EvenementController {
  ...
  async update(
    ...
    const existingEvent = await this.evenementService.findOneById(+id);
    if (existingEvent.user.id !== userIdFromToken) {
    ...
    if (file) {
      ...
      if (existingEvent.imageUrl) {
        const oldFilePath = path.join(__dirname, '../../../', existingEvent.imageUrl);
    ...
    try {
      return await this.evenementService.update(+id, dto);
    } catch (error) {
      if (error.code === '23505') {
        throw new BadRequestException("Un autre événement avec ce nom existe déjà.");
      }
