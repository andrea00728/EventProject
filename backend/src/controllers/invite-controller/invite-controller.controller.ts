 ...
 */
  ...
  async verifyEmail(
    ...
    if (!email) throw new HttpException('Email requis', HttpStatus.BAD_REQUEST);
    const invite = await this.guestService.findByEmail(email, eventId);
    if (!invite) throw new HttpException('Invité non trouvé', HttpStatus.NOT_FOUND);
    ...
    return {
      ...
      email: invite.email,
      table: invite.table?.numero,
      place: invite.place,
      eventName: invite.event.nom,
    };
  }
