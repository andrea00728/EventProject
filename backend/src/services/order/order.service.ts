...
import { Repository, DeepPartial, Like, DataSource } from 'typeorm';
...
export class OrderService {
  ...
  async createOrder(tableId: number, items: { menuItemId: number; quantity: number }[], nom?: string, email?: string): Promise<Order> {
    return await this.dataSource.transaction(async (manager) => {
      ...
      const table = await manager.getRepository(TableEvent).findOne({ where: { id: tableId }, relations: ['event'] });
      if (!table) {
      ...
      const orderItems = await Promise.all(
        items.map(async (item) => {
          ...
          await manager.getRepository(MenuItem).save(menuItem);
      ...
      savedOrder.items = await manager.getRepository(OrderItem).save(orderItems);
      const finalOrder = await manager.getRepository(Order).save(savedOrder);
  ...
  async cancelOrder(orderId: number, userId: string): Promise<{ message: string }> {
    return await this.dataSource.transaction(async (manager) => {
      ...
      if (order.payments && order.payments.length > 0) {
        console.log(`Marquage des paiements comme remboursés pour orderId: ${orderId}`);
      ...
      if (order.items && order.items.length > 0) {
        for (const orderItem of order.items) {
          ...
          const menuItem = await manager.getRepository(MenuItem).findOne({ where: { id: orderItem.menuItem.id } });
          if (!menuItem) {
      ...
      if (order.paymentStatus === 'paid' && order.table?.event) {
        ...
        const paidOrders = await manager.getRepository(Order).find({
          where: {
            paymentStatus: 'paid',
        ...
        let balance = await manager.getRepository(Balance).findOne({ where: { eventId } });
        if (balance) {
  ...
  async refundOrder(orderId: number, userId: string, refundReason: string): Promise<{ message: string }> {
    return await this.dataSource.transaction(async (manager) => {
      console.log(`Début de refundOrder pour orderId: ${orderId}, userId: ${userId}`);
      const user = await manager.getRepository(User).findOne({ where: { id: userId } });
      if (!user || user.role !== 'caissier') {
        console.error(`Accès non autorisé pour userId: ${userId}`);
        throw new UnauthorizedException('Seul un caissier peut rembourser une commande');
      }
      ...
      this.ordersGateway.handleOrderRefunded({ id: orderId, paymentStatus: 'refunded' });
  ...
  async updateOrder(orderId: number, tableId: number, items: { menuItemId: number; quantity: number }[]): Promise<Order> {
    return await this.dataSource.transaction(async (manager) => {
      ...
      order.items = await manager.getRepository(OrderItem).save(newOrderItems);

      if (order.table.id !== tableId) {
        const newTable = await manager.getRepository(TableEvent).findOne({ where: { id: tableId } });
        if (!newTable) {
          console.error(`Tableau non trouvé pour tableId: ${tableId}`);
          throw new NotFoundException('Tableau non trouvé');
        }
  ...
  async validatePayment(orderId: number, email: string): Promise<Order> {
    return await this.dataSource.transaction(async (manager) => {
      ...
      const personnel = await manager.getRepository(Personnel).findOne({ where: { email: email } });
      ...
      if (!balance) {
        balance = manager.getRepository(Balance).create({
          total: updatedTotal,
          updatedAt: new Date(),
          event: order.table.event,
          eventId,
  ...
  async getRefundedOrders(eventId: number, userId: string): Promise<(Order & { total: number })[]> {
    ...
    const orders = await this.orderRepository.find({
      ...
      relations: ['items', 'items.menuItem', 'table', 'table.event', 'refundedBy'],
    });
