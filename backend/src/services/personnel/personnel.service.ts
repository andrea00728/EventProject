...
async confirmEmail(token: string): Promise<string> {
  try {
   ...
   const personnel = await this.personnelRepository
    ...
    const accept=await this.personnelRepository.save(personnel);
    await this.notificationService.notifyAll(
      'invitation accepter',
      `l'email de l'utilisateur est ${personnel.email} accepter votre invitation de devenir ${personnel.role} de l'evenement ${personnel.evenement.nom}`,
    )
